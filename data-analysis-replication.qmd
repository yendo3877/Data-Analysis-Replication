...
First, I renamed variances in data set to match with published paper and converted genotype and temperature to factors for statistical analysis and ploting. I also grouped genotype and temperature to group.
...
As data set contains 12 genotypes and 2 temperature treatment, after grouping, I got 24 groups of data that I will perform descriptive statistical analysis.
...
Then I peformed Tukey HSD test to see: "Which specific genotype-temperature groups are significantly different from each other?"
...
Tukey HSD post-hoc comparisons revealed that many genotype-temperature combinations differ significantly. For instance, lhp1-3, ein2-5 lhp1-3, and ein3-1 eil1-1 lhp1-3 mutants had significantly shorter hypocotyls than wild-type (wt) at 21°C (p < 0.01). Most genotypes showed significantly longer hypocotyls at 27°C compared to 21°C, indicating strong temperature responsiveness. However, some mutants (e.g. ein2-5 lhp1-3) showed less elongation, suggesting impaired thermoresponsive growth.
...
Conclusion: Warm temperature (27°C) significantly promotes hypocotyl elongation in wild-type (Col-0) and several genotypes. However, mutants with disrupted ethylene signaling (ein2, ein3-1 eil1-1) or chromatin regulation (lhp1) show impaired thermoresponsive elongation, especially in combinations like ein2-5 lhp1-3 and ein3-1 eil1-1 lhp1-3.

Introduction of the proLHP1::gLHP1-GFP transgene in these mutant backgrounds restores hypocotyl elongation, often to wild-type levels. This indicates that LHP1 is necessary and sufficient for thermomorphogenic growth, and that it can compensate for defects in ethylene signaling when overexpressed.
...
To firgure out all single mutants involve in some same pathway, I filtered out genes pool that lower expression in mutants when exposed to warm-temperature.
...
Using (-4 to 8) with my samples results in all blue color in heatmap, makes it harder to indentify lower expression gene. Therefore, I changed it to -4 to 8.
...
RNA-seq analysis can be widely used to study gene expression profiling, differential gene expression, and pathway or functional enrichment analysis. In the original paper, RNA-seq data was also applied to many other types of analyses. Here, due to my current limitations, I only reported a small portion of them. However, I now have a better understanding of how to handle the dataset and use R to compare expression patterns among experimental groups. This will be a starting point for my future RNA-seq analyses.
